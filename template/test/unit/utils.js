 ...
 * 等待一个事件
 */
export const waitForEvent = (vm, eventName, ms = 1000) => {
  return new Promise((resolve, reject) => {
    let t
    vm.$once(eventName, (evt) => {
      t && clearTimeout(t)
      resolve(evt)
    })

    t = setTimeout(() => {
      reject(new Error(`wait event ${eventName} timeout at ${ms}ms`))
    }, ms)
  })
}

export const Store = (data) => new Vuex.Store(data)
