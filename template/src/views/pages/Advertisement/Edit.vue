    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true">
        <el-form-item>
          ...
          <el-button v-if='form.status==="OFFLINE"' type="success" @click="saveStatus('ONLINE')">上线</el-button>
          <el-button v-else type="danger" @click="saveStatus('OFFLINE')">下线</el-button>
        </el-form-item>
      </el-form>
    </el-col>
    <el-col :span="24">
          ...
          <el-col :span="18">
            ...
            <div v-show="form.type==='VIDEO'">
              ...
              <el-form-item label="视频上传" prop="filePath">
                <el-upload ref='videoUpload' class="upload-demo" drag :action="uploadPath" :headers="uploadHeaders" accept="video/*" :limit="1" :on-success="handleUploadVideoSuccess" :on-remove="handleUploadVideoRemove" :file-list="videoList">
            ...
            <div v-show="form.type==='IMAGE'">
              <el-form-item label="图片上传" prop="filePath">
                <el-upload ref='imageUpload' :action="uploadPath" :headers="uploadHeaders" list-type="picture-card" :multiple="false" :limit="1" :on-success="handleUploadImageSuccess" :on-preview="handleUploadImagePreview" :on-remove="handleUploadImageRemove" :file-list="imageList" accept="image/*">
...
<script>
import { ADRatioType, ADType, ADTimeSlot, ADShowType, Status } from 'src/lib/enum'
  ...
  methods: {
    ...
    saveSubmit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          adService.updataAD(this.form).then(() => {
            // 更新列表
            this.$message({
              message: '保存成功',
              type: 'success'
    ...
    saveStatus(status) {
      adService.updateADStatus(this.id, status).then(() => {
        this.$message({
          message: '更新成功',
          type: 'success'
        });
    ...
    getAD() {
      adService.getAD(this.id).then((data) => {
        this.form = data;
        if (this.form.filePath) {
          if (this.form.type === 'IMAGE') {
            // fileList: [{name: 'food.jpeg', url: 'https://fuss10.elemecdn.com/3/63/4e7f3a15429bfda99bce42a18cdd1jpeg.jpeg?imageMogr2/thumbnail/360x360/format/webp/quality/100'}]
            this.imageList = [{ name: this.form.filename, url: this.form.fileUrl }]
          } else if (this.form.type === 'VIDEO') {
            this.videoList = [{ name: this.form.filename, url: this.form.fileUrl }]
          }
        }
      })
    },
